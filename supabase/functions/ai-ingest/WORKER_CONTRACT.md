...
O worker deve usar credenciais de serviço apenas no ambiente do worker e nunca expô-las no cliente. O contrato SQL da migração 0018 já fornece USN, RLS e tombstones para os jobs e para as entidades materializadas. O claim deve possuir timeout/retry para que um job preso em `processing` possa ser recuperado por operação administrativa sem duplicar notas ou cartões.
